		       ...
		       const bool submit)
{
	...
	double target_shares_4min = ckp->shares_per_minute * 4.0;  // 4 minutes worth of shares
	if (client->ssdc < target_shares_4min && tdiff < 240)
		return;
	...
	/* Calculate target DRR based on shares_per_minute and allow hysteresis. */
	double target_sps = ckp->shares_per_minute / 60.0;  // Convert to shares per second
	double lower_bound = target_sps * 0.5;   // 50% below target
	double upper_bound = target_sps * 1.33;  // 33% above target
	if (drr > lower_bound && drr < upper_bound)
		return;
	...
	if (mindiff) {
		if (drr < target_sps * 0.8)  // Allow slightly lower threshold for mindiff
			return;
